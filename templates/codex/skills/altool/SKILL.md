...
4. Follow `_common.md` and `altool/standards.md` for product-policy validation, AI-maintained project standards, and every Step Check. Read relevant originals, not only Spec summaries; pass the router and sources to independent reviewers. Refresh the owner check for every changed Altool document, list those checks in `docs.synced`, and run `check.py audit-docs --root .` after document-producing commands.
...
6. Follow the constitution's design authority and the shared design-input discovery/synchronization procedure in `altool/standards.md`; command steps own automatic execution.
