...
| `python altool/scripts/freedom_loop.py ...` | Freedom control-plane smoke runner |
| `$altool say/ask <text>` | `python altool/scripts/radio.py say/ask <text>` |
...
1. Before executing a command, read root `AGENTS.md`, the complete `constitution.md`, and the matching step. This skill routes commands; the step is the complete command procedure.
2. Keep Altool state, artifacts, reports, skills, and prompts project-local. Missing optional assets disable only their rules; never create global prompt, skill, or command files.
...
4. Every command must write and validate its Step Check. Refresh the owner check for every changed Altool document, list those checks in `docs.synced`, and run `check.py audit-docs --root .` after document-producing commands. Use `done` with evidence or `skipped/failed` with a reason; repair validation failures up to the step's retry limit.
