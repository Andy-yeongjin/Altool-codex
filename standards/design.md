...
> `standards/design.md`는 직접 디자인 원천을 정규화한 단일 구현 계약입니다. Claude 디자인 HTML이 있으면 HTML이 최상위 원천입니다.
...
3. 없는 항목은 기존 의미/변형을 먼저 대조합니다. 진짜 공통 요구면 회사 공통 팩 제작 원본에 등록·검증하고 새 버전으로 배포합니다. 기능 전용 콘텐츠만 기능 Spec에 범위·이유를 남깁니다. 기존 의미를 새 ID로 우회하거나 로컬 lock을 재작성해 통과시키지 않습니다.
4. 같은 회사의 프로젝트는 같은 배포 원본·release/lock을 사용합니다. 개별 프로젝트에서 의미를 변경하지 않습니다. 회사 CI·문구 변경은 공통 팩 버전 변경과 소비 앱 영향 검증으로 전파합니다. 로컬 파일 배포가 타 프로젝트를 자동 갱신하는 것은 아닙니다.
5. `.altool/asset-usage/{feature}.json`에 변경 UI 파일·사용 ID/변형·실제 참조·복사 배포 경로 및 기능 전용 근거를 남깁니다. `altool/standards.md`의 UI 적용 증거 절차에 따라 라우팅과 지침별 실제 검증을 연결하고 `assets.py evidence --usage ...`를 통과시킵니다. Analyze/Browser는 누락된 의미와 임의 문구/아이콘을 독립 대조합니다. 구조·hash 검사는 전체 의미 준수의 증명이 아닙니다.

| 역할 | 원본 경로·출처 | 실행 코드 경로 | 변형·상태·대체 텍스트 | 라이선스·검증 |
| --- | --- | --- | --- | --- |
| {실제 사용한 공통 자산} | {semantic ID / release / 파일 / 출처} | {앱이 읽는 위치} | {허용 variant·크기·alt 또는 장식} | {사용 조건·실제 검증} |
...
2. standard.yaml에서 지정한 항목을 읽습니다. 기본 design 경로는 전체 계약(*)이며, 프로젝트가 항목을 나눌 때는 필요한 화면·컴포넌트 의존 항목도 함께 연결합니다.
