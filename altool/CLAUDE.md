...
| `/al analyze {기능명}` | `altool/steps/analyze.md` | `docs/03-analyze/{기능명}.analyze.md` |
| `/al fix {기능명}` | `altool/steps/fix.md` | 개선 코드 + `docs/03-analyze/{기능명}.fix.md` |
...
| analyze | `altool/templates/analyze.template.md` |
...
- **90% 이상 = 통과선** (report 진행 가능 기준). 단 fix 진입/종료는 **미해소 갭 존재 여부** 기준 — Fix 규칙 참조.
...
- **buildVerified**: run/fix가 프로덕션 빌드 성공을 확인하면 `true`로 기록 — analyze·report가 빌드 성공의 독립 증거로 사용한다 (문서 체크박스만으로는 검증 불가).
...
- **갱신 방법**: 상태 파일은 **문자열 치환으로 수정 금지** — 같은 키가 features와 history 양쪽에 있어 이력이 오염된다 (L-007 실증). 반드시 전체를 올바른 구조로 재작성하거나 파싱 후 수정한다.
- 단계를 건너뛰고 실행하면 (예: plan 없이 design) → 경고 후 사용자에게 진행 여부 확인.

## Fix 규칙

- 실행 조건: **analyze 문서 §3에 미해소 갭 존재** — Match Rate ≥ 90%여도 갭이 있으면 Checkpoint 5 결정 범위대로 수정한다 (90%는 "통과선"이지 "수정 면제선"이 아님)
- 미해소 갭 0건이면 → "개선 불필요" 안내 후 종료
- 수정 → **자동 재분석(re-Check)** → 갭 0건 도달 또는 **최대 5회** 반복 시 중단 (`/al oneshot` 내부에서는 2회 후 사용자 확인)
...
| fix | analyze 문서의 갭 목록의 해소 항목 표기 |
| report | plan §9 잔여 항목, analyze 문서 §8, 문서 Status `Draft` → `Finalized` |
