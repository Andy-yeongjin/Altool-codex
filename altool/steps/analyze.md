...
`altool/templates/analyze.template.md`를 Read 도구로 읽고 그 절 구조를 아웃라인으로 사용한다.
...
- Design → 구조적 구현 일치
- Design의 Context Anchor를 analyze 문서 헤더에 복사
...
**Altool 자산 감지 — 디자인 토큰 검증** (`designs/design-tokens.css`가 있을 때만, analyze 문서에 절 추가):
| 항목 | 기준 |
|------|------|
...
- 미해소 갭 0건 → `다음 단계: /al report {기능명}`
- 갭 존재 → `다음 단계: /al fix {기능명}` (Match Rate ≥90%여도)
