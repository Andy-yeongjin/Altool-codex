> 지침: 모든 내용은 반드시 한글로 작성합니다 (헌법의 언어 원칙).

# {기능명} — 개발 계획서 (Plan)

> **요약**: {이 기능을 한 줄로}
>
> **프로젝트**: {프로젝트명} | **작성일**: YYYY-MM-DD | **작성**: Altool
> **상태**: Draft <!-- Step Check 통과 후 Planned, spec/run/analyze/browser/report 단계에서 Specified/Implemented/Analyzed/Verified/Completed로 갱신 -->
> **입력 자산**: Research {docs/00-research/ 또는 —} · PRD {경로 또는 —} · 참고 문서 {prd/refs/ 또는 —} · 디자인 원본 {.pen/Stitch/스크린샷/디자인 문서 또는 —}

---

## Executive Summary

| 관점 | 내용 |
|------|------|
| **문제 (Problem)** | {이 기능이 해결하는 핵심 문제} |
| **해법 (Solution)** | {선택한 접근} |
| **기능·UX 효과** | {사용자가 체감하는 변화} |
| **핵심 가치** | {한 문장 가치 제안} |

## Context Anchor

> Executive Summary·요구사항·리스크에서 추출. spec → run → analyze로 전파되어 전략적 맥락을 유지한다.

| Key | Value |
|-----|-------|
| **WHY** | {핵심 문제} |
| **WHO** | {타겟 사용자} |
| **RISK** | {최대 리스크 1개 + 대응 방향} |
| **SUCCESS** | {측정 가능한 성공 기준 요약} |
| **SCOPE** | {범위 한 줄 + 범위 외 한 줄} |

---

## 1. 개요

### 1.1 목적
{이 기능이 달성하려는 목표}

### 1.2 배경
{왜 지금 이 기능이 필요한가 — 비즈니스/사용자 맥락}

### 1.3 관련 문서
- Research: {관련 research ID/경로 또는 "없음 — 기능 설명에서 도출"}
- PRD: {경로 또는 "없음 — 기능 설명에서 도출"}
- 기준: `constitution.md`, `designs/claude-design/**/*.html`, 기타 사용자 디자인 입력, `standards/design.md`, Research

### 1.4 디자인 입력 우선순위

| 우선순위 | 입력 | 감지 경로 | Plan 처리 |
| ---: | --- | --- | --- |
| 1 | Claude 디자인 HTML | `{designs/claude-design/**/*.html 또는 없음}` | {최상위 구조·시각·상태 원천 또는 없음} |
| 2 | 기타 사용자 디자인 입력 | `{designs/**/*.pen, designs/stitch/, screenshots, design docs 또는 없음}` | {화면 구조·시각 의도의 직접 원천 또는 없음} |
| 3 | 프로젝트 디자인 시스템 | `{standards/design.md 또는 없음}` | {상위 원천을 정규화한 구현 계약 또는 없음} |
| 4 | Research | `{관련 R-ID 또는 없음}` | {상위 원천의 빈칸 보강 근거} |
| 5 | AI 자체 판단 | — | {미정 세부 보완만 허용} |

> `constitution.md`의 디자인 품질 원칙은 위 우선순위와 별개로 항상 적용한다.

### 1.5 Research 반영

| Research ID | 반영한 관찰 | Plan 반영 위치 |
| --- | --- | --- |
| {R-0001 또는 없음} | {공통 패턴/리스크/후보 기능} | {FR/SC/리스크/범위} |

### 1.6 PRD/Research 대조

> PRD가 있으면 PRD가 기준 계약이다. Research는 PRD 구현을 더 잘하기 위한 보강재로만 사용한다.

| 항목 | PRD 기준 | Research 제안 | 처리 | 사유 |
| --- | --- | --- | --- | --- |
| {요구/후보} | {PRD에 명시된 범위} | {Research 관찰/후보} | 반영/보류/제외 | {PRD 우선, UX 보강, 다음 사이클 등} |

---

## 2. 범위

### 2.1 포함 (In Scope)

> run 완료 시 구현된 항목을 `- [x]`로 갱신한다 (문서 동기화 규칙).

- [ ] {포함 항목 1 — 공개 사용자용 웹서비스면 홈 또는 핵심 진입 경험을 우선하고, 다른 프로젝트 유형이면 해당 없음으로 표시}
- [ ] {포함 항목 2}

### 2.2 제외 (Out of Scope)

> 의도적으로 다루지 않는 것. 여기 적힌 항목은 analyze에서 갭으로 잡지 않는다.

- {제외 항목 + 한 줄 사유}

---

## 3. 요구사항

### 3.1 기능 요구사항 (FR)

> Status는 run 완료 시 `Pending` → `✅ 완료`로 갱신한다.

| ID | 요구사항 | 우선순위 | Status |
|----|---------|:-------:|:------:|
| FR-01 | {구현·검증 가능한 문장으로} | High | Pending |
| FR-02 | | Medium | Pending |

### 3.2 비기능 요구사항 (NFR)

| 분류 | 기준 | 측정 방법 |
|------|------|----------|
| 품질 | 선택한 스택의 타입·정적 검사, 프로덕션 빌드가 있으면 성공 | 프로젝트 build/typecheck |
| 접근성 | 읽기 쉬운 대비, 충분한 조작 영역, 키보드 사용 가능, 명확한 포커스와 상태 표현 (헌법의 범용 UI 품질) | 코드 검사 + L2 |
| 반응형 | UI가 있으면 모바일 375px 이상과 데스크톱에서 정상 표시 | L2 |
| {성능 등 추가} | | |

---

## 4. 성공 기준 (Success Criteria)

> **analyze가 검증 가능한 형태**로 작성한다 — 각 SC는 증거(file:line, 테스트 결과, 측정값)로 판정할 수 있어야 한다.
> analyze에서 충족 확인된 항목만 `- [x]`로 갱신한다.

- [ ] SC-1: FR 전 항목 구현
- [ ] SC-2: 사용자 디자인 입력이 있으면 그 화면 구조와 시각 기준을 우선 반영. UI 구현은 `standards/design.md`를 기준으로 하며 색상·타이포·간격·둥글기·그림자·컴포넌트·미디어 규칙의 근거가 추적 가능해야 한다. 디자인 시스템이 없으면 Research 또는 design_source에서 `design.md`를 생성한 뒤 진행
- [ ] SC-3: 프로덕션 빌드 성공
- [ ] SC-4: {기능 고유의 핵심 기준}

---

## 5. 리스크와 대응

| 리스크 | 영향 | 가능성 | 대응 |
|--------|:----:|:-----:|------|
| {기술·일정·외부 의존 리스크} | High/Med/Low | High/Med/Low | {구체적 회피·완화책} |

---

## 6. 영향 분석 (Impact Analysis)

> **변경되는 리소스의 기존 사용처를 전수 조사**한다. 기존 기능을 깨뜨리는 변경을 사전에 잡는 절이다.
> 신규 프로젝트면 "신규 — 영향 받는 기존 소비자 없음"으로 표기하고 종료.

### 6.1 변경 리소스

| 리소스 | 유형 (모델/API/설정/페이지) | 변경 내용 |
|--------|---------------------------|----------|
| | | |

### 6.2 기존 소비자

| 리소스 | 사용 코드 경로 | 영향 (없음/확인 필요/파괴적) |
|--------|--------------|---------------------------|
| | | |

### 6.3 회귀 확인

- [ ] 위 소비자 전부가 변경 후에도 정상 동작함을 확인 (run 완료 시 체크)

---

## 7. 아키텍처 방향

### 7.1 프로젝트 레벨

| 레벨 | 특징 | 적합 | 선택 |
|------|------|------|:----:|
| **Starter** | 단순 구조 (components/lib/types) | 정적 사이트, 단일 화면 도구 | ☐ |
| **Dynamic** | 기능 모듈 + API/DB | 백엔드 있는 웹앱, 완성형 서비스 | ☐ |
| **Enterprise** | 엄격한 레이어 분리 | 대규모·복잡 도메인 | ☐ |

### 7.2 핵심 기술 결정

> 미정 항목은 spec 단계의 아키텍처 3안 비교로 넘긴다. 중요하고 되돌리기 어려운 결정은 사유를 ADR로 남긴다 (헌법의 DDD·ADR 원칙).

| 결정 | 선택 | 사유 |
|------|------|------|
| Framework | {기존 프레임워크 / 새 동적 웹앱이면 Next.js App Router 기본값 / 정적 결과물이면 없음} | 헌법의 기술 스택 기본값 |
| Language | {기존 프로젝트 언어 / 새 동적 웹앱이면 TypeScript Strict 기본값} | 헌법의 기술 스택 기본값 |
| DB | {기존 DB / SQLite(로컬) / 없음} | 영속 데이터 필요 여부와 헌법의 기술 스택 기본값 |
| 상태 관리 | {useState / Context / …} | |
| Styling | {기존 방식 / 새 프로젝트면 CSS Modules 기본값} + {디자인 원천 + design.md} | 헌법의 디자인 권위 순서 적용 |

---

## 8. 컨벤션·환경

- 명명: 선택한 생태계 관례 + 도메인 의도 명명 (헌법의 DDD 원칙)
- 환경 변수: {목록 또는 "없음"} — 비밀 정보는 `.env` 또는 승인된 Secret Manager로 격리
- 기존 프로젝트 컨벤션과 충돌 시: {처리 방침}

---

## 9. 다음 단계

> 각 단계 완료 시 해당 항목을 `- [x]`로 갱신하고, 문서 상단 `상태`도 현재 단계에 맞춰 갱신한다.

1. [ ] 구현 명세 (`$altool spec`)
2. [ ] 구현 (`$altool run`)
3. [ ] 갭 분석 (`$altool analyze`)

---

## Version History

| Version | Date | Changes | Author |
|---------|------|---------|--------|
| 0.1 | YYYY-MM-DD | 최초 작성 | Altool |
