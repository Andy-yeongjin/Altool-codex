...
(function () {
  ...
  const source = page => `<a class="source" href="../../../ATTRIBUTION.md">참고출처 · 가이드라인(2024.02) p.${page}부터</a>`;
  ...
  function stepList(labels, index) { return `<ol class="steps ui-steps" aria-label="진행 단계">${labels.map((label, i) => `<li${i === index ? ' aria-current="step"' : ''}>${i + 1}. ${e(label)}${i === index ? ' · 현재' : ''}</li>`).join('')}</ol>`; }
  ...
  function visit() {
    main.innerHTML = pageHeading('필요한 서비스를 찾아보세요', '방문 → 정보 확인 → 이동을 연결하는 첫 화면입니다. 모든 내용과 날짜는 예제이며 실제 정책 안내가 아닙니다.', 686) + `
      <aside class="notice ui-alert" aria-label="중요 안내"><strong>실습 안내</strong><p>실제 개인정보를 입력하지 마세요. 신청 기록은 이 브라우저의 고정 선택값만 저장합니다.</p></aside>
      <h2>자주 찾는 서비스</h2><p>샘플 우선순위입니다. 실서비스에서는 이용자 조사와 이용 데이터로 순서를 결정하세요.</p>
      <div class="cards"><article class="card ui-summary"><h3><a href="#application">교육 참여 모의 신청</a></h3><p>대상 확인부터 신청 내역까지 연습합니다.</p><a class="button" href="#application/eligibility">모의 신청 시작</a></article><article class="card ui-summary"><h3><a href="#policy">정책 정보</a></h3><p>분야별 가상 정책과 관련 자료를 살펴봅니다.</p></article><article class="card ui-summary"><h3><a href="#search">통합 검색</a></h3><p>정책·서비스·자료를 한 번에 검색합니다.</p></article></div>
  ...
  function renderSearch() {
    main.innerHTML = pageHeading('통합 검색', '가상 데이터 15건에서 검색합니다. 검색어·필터는 상세 화면을 다녀와도 현재 탭에서 유지됩니다.', 702) + `
      <form id="search-form" role="search" class="search-box ui-filter-bar"><div class="field ui-field"><label for="search-query">검색어</label><input id="search-query" type="search" value="${e(searchState.query)}" aria-describedby="search-help" autocomplete="off" aria-controls="suggestions" aria-expanded="false"><p id="search-help" class="muted">예: 배움, 청년, 생활 안전. 실습 기준일은 2024-02-29입니다.</p></div><div id="suggestion-region" hidden><h2 id="suggestion-title">추천 검색어</h2><p id="suggestion-status" role="status" aria-live="polite"></p><ul id="suggestions" class="suggestions" aria-labelledby="suggestion-title"></ul></div><div class="ui-filter-actions"><button type="submit" class="primary">검색</button><button type="button" id="clear-query">검색어 전체 삭제</button></div></form>
      <div class="ui-filter-bar" aria-label="상세 검색"><div class="field ui-field"><label for="search-period">검색 기간</label><select id="search-period" data-ui-select><option value="all">전체</option><option value="1">1일</option><option value="7">일주일</option><option value="30">1개월</option><option value="custom">직접 지정</option></select></div><div class="field ui-field"><label for="search-category">자료 유형</label><select id="search-category" data-ui-select>${['전체', '정책', '서비스', '자료'].map(v => `<option>${v}</option>`).join('')}</select></div><div class="field ui-field"><label for="search-sort">정렬 기준</label><select id="search-sort" data-ui-select><option value="relevance">관련도순</option><option value="latest">최신순</option><option value="popular">인기순</option></select></div><div class="ui-filter-actions"><button id="reset-filters">검색 조건 전체 초기화</button></div></div>
      <div id="custom-range"${searchState.period === 'custom' ? '' : ' hidden'} class="ui-filter-bar"><div class="field ui-field"><label for="search-from">시작 날짜</label><input type="date" id="search-from" value="${e(searchState.from)}"></div><div class="field ui-field"><label for="search-to">종료 날짜</label><input type="date" id="search-to" value="${e(searchState.to)}"></div><div class="ui-filter-actions"><button id="apply-range">날짜 적용</button></div></div>
  ...
  function displayResults() {
    ...
    byId('search-results').innerHTML = `<div class="filter-buttons" role="group" aria-label="검색 주제">${counts.map(c => `<button data-category="${c.category}" aria-pressed="${searchState.category === c.category}">${c.category} (${c.count})</button>`).join('')}</div>` + (data.length ? `<ul class="results">${data.slice((searchState.page - 1) * 10, searchState.page * 10).map(item => `<li class="result ui-section"><span class="badge">${item.category}</span><time>${item.date}</time><h3><a href="#search/detail/${item.id}">${highlighted(item.title, searchState.query)}</a></h3><p class="summary">${highlighted(item.description, searchState.query)}</p><div class="result-meta">실습실 &gt; ${item.category} &gt; ${e(item.title)}</div></li>`).join('')}</ul><nav class="pagination ui-pagination" aria-label="검색 결과 페이지">${Array.from({ length: pages }, (_, i) => `<button data-page="${i + 1}"${searchState.page === i + 1 ? ' aria-current="page"' : ''}>${i + 1}</button>`).join('')}</nav>` : '<div class="notice ui-alert"><h2>검색 결과가 없습니다</h2><p>철자를 확인하거나 다른 단어·더 일반적인 단어로 검색해 보세요. 검색 조건을 초기화할 수도 있습니다.</p><button id="empty-reset">모든 검색 조건 해제</button></div>');
  ...
  function login(stage) {
    ...
    if (stage === 'entry') {
      main.innerHTML = pageHeading('모의 로그인 정보 입력', '아래 공개된 예제 값만 입력하세요. 실제 비밀번호·이메일·개인정보를 입력하지 마세요.', 819) + stepList(loginSteps, 1) + (method === 'sample' ? `<form id="login-form" novalidate><p class="notice ui-alert">예제 아이디: <strong>demo@example.invalid</strong><br>예제 비밀번호: <strong>sample-only</strong></p><div class="field ui-field"><label for="demo-id">예제 아이디 (이메일 형식)</label><small id="id-help">실제 이메일 주소를 사용하지 마세요.</small><input id="demo-id" type="email" autocomplete="username" aria-describedby="id-help login-error"></div><div class="field ui-field"><label for="demo-password">예제 비밀번호</label><input id="demo-password" type="password" autocomplete="current-password" aria-describedby="login-error"><button type="button" id="show-password" aria-pressed="false">비밀번호 표시</button></div><label class="choice"><input id="remember-method" type="checkbox">이 탭에서 로그인 방식만 기억 (계정 정보는 저장하지 않음)</label><p id="login-error" class="error" role="alert" hidden></p><div class="actions ui-form-actions"><a class="button" href="#login">다른 방식 선택</a><button class="primary" type="submit">모의 로그인</button></div></form><details><summary>아이디·비밀번호 찾기 도움</summary><p>이 예제에는 계정이 없습니다. 화면에 제공된 예제 값만 사용하세요. 실제 서비스에서는 계정 복구 화면·접근 가능한 대체 인증을 연결해야 합니다.</p></details>` : '<p class="notice ui-alert">기억·계산·문자열 입력이 없는 대체 흐름의 화면 예제입니다. 생체 정보나 인증서를 수집하지 않습니다.</p><div class="actions ui-form-actions"><a class="button" href="#login">다른 방식 선택</a><button class="primary" id="login-alternative">대체 방식 모의 완료</button></div>');
    ...
    main.innerHTML = pageHeading('모의 로그인 방식 선택', '방식을 비교한 후 선택합니다. 두 방식 모두 같은 실습 메뉴를 제공하며 법적 효력·인증 권한이 없습니다.', 803) + stepList(loginSteps, 0) + `${preferredMethod ? '<p class="notice ui-alert">이 탭에서 사용한 방식: 예제 아이디 입력</p>' : ''}<div class="cards"><section class="card ui-summary"><h2>예제 아이디 입력</h2><p>공개된 샘플 아이디와 비밀번호로 양식·오류 상태를 연습합니다.</p><button id="method-sample" class="primary">예제 아이디 방식 선택</button></section><section class="card ui-summary"><h2>정보 입력 없는 대체 방식</h2><p>개인정보 입력 없이 대체 인증의 완료 화면을 살펴봅니다. 실제 인증은 실행하지 않습니다.</p><button id="method-alternative">대체 방식 선택</button></section></div><details><summary>모바일·키보드 이용 도움말</summary><p>화면 크기와 관계없이 두 방식을 목록으로 비교할 수 있습니다. Tab으로 선택 버튼에 이동하고 Enter로 다음 화면을 여세요. 붙여넣기는 제한하지 않습니다.</p></details>${source(785)}${source(793)}`;
  ...
  function updateHeader() {
    ...
    link.innerHTML = '<img src="../../dist/assets/icons/user.svg" alt="" width="24" height="24"> ' + (signedIn ? '모의 계정' : '로그인');
  ...
  function application(stage, id) {
    ...
    if (stage === 'eligibility') {
      main.innerHTML = pageHeading('시작하기 전에', '모의 교육 참여 신청입니다. 실제 신청 자격이나 법적 동의를 판단하지 않습니다.', 894) + stepList(applicationSteps, 0) + '<div class="notice ui-alert"><h2>미리 확인하세요</h2><ul><li>이 예제는 누구나 연습할 수 있습니다.</li><li>실제 본인 인증·수수료·증빙서류는 없습니다.</li><li>모의 접수 뒤 수정은 하지 않으며 신청 내역에서 모의 취소할 수 있습니다.</li></ul></div><form id="eligibility-form"><fieldset><legend>예제 자격에 해당하나요?</legend><label class="choice"><input type="radio" name="eligible" id="eligible" value="yes">예, 연습용 조건에 해당합니다</label><label class="choice"><input type="radio" name="eligible" value="no">아니요, 자격 미달 안내를 시험합니다</label></fieldset><p id="application-error" class="error" role="alert" hidden></p><div class="actions ui-form-actions"><a class="button" href="#application">목록으로 돌아가기</a><button class="primary">다음: 신청서 작성</button></div></form>';
    ...
    if (stage === 'form') {
      ...
      main.innerHTML = pageHeading('모의 교육 참여 신청서', '개인정보 대신 고정된 예제 선택값만 사용합니다. 필수 항목을 모두 선택해 주세요.', 903) + stepList(applicationSteps, 1) + `<form id="application-form" novalidate><h2>참여 정보</h2><div class="field ui-field"><label for="topic">교육 주제 (필수)</label><select id="topic" aria-describedby="application-error" data-ui-select><option value="">선택해 주세요</option><option>디지털 기초</option><option>생활 안전</option></select></div><div class="field ui-field"><label for="delivery">참여 방법 (필수)</label><small id="delivery-help">방문을 선택하면 시간대를 추가로 선택합니다. 온라인으로 변경해도 이전 시간 선택은 저장되지만 제출에는 사용되지 않습니다.</small><select id="delivery" aria-describedby="delivery-help application-error" data-ui-select><option value="">선택해 주세요</option><option>온라인</option><option>방문</option></select></div><div class="field ui-field" id="time-field" hidden><label for="time">방문 시간대 (필수)</label><select id="time" aria-describedby="application-error" data-ui-select><option value="">선택해 주세요</option><option>오전</option><option>오후</option></select></div><p id="dependency-notice" role="status"></p><details><summary>작성 도움말</summary><p>주제는 원하는 교육 예제를 뜻합니다. 방문을 선택하면 시간대가 필요합니다. 이 샘플에는 업로드할 증빙 서류가 없습니다. 실제 서비스에서는 기관 담당자 문의처와 정확한 작성 매뉴얼을 연결하세요.</p></details><p id="application-error" class="error" role="alert" hidden></p><div class="actions ui-form-actions"><a class="button" href="#application/eligibility">이전: 유의 사항·자격</a><button type="button" id="save-draft">모의 임시 저장</button><button class="primary" type="submit">다음: 확인·확정</button></div></form><div class="secondary-zone"><button class="danger" id="reset-draft">이 모의 신청서 초기화</button><p id="draft-saved-at">${savedAt ? `마지막 임시 저장: ${e(M.formatDate(savedAt))}` : '아직 임시 저장하지 않았습니다.'} 저장값은 이 브라우저에 남습니다.</p></div>`;
    ...
    if (stage === 'review') {
      ...
      main.innerHTML = pageHeading('신청 내용을 확인해 주세요', '모의 접수 후 이 기록의 내용은 수정할 수 없습니다. 신청 내역에서 모의 취소 후 다시 연습할 수 있습니다.', 918) + stepList(applicationSteps, 2) + summary(draft) + '<div class="actions ui-form-actions"><a class="button" href="#application/form">돌아가서 수정</a><button class="primary" id="submit-application">모의 신청 확정</button></div><p id="submit-status" role="status"></p>';
  ...
  function history() {
    ...
    main.innerHTML = pageHeading('신청 내역', '이 브라우저에 저장한 모의 내역입니다. 실제 신청 이력·심사 결과가 아닙니다.', 934) + `<div class="ui-filter-bar"><div class="field ui-field"><label for="receipt-filter">진행 상태</label><select id="receipt-filter" data-ui-select><option>전체</option><option>모의 접수</option><option>모의 취소</option></select></div><div class="field ui-field"><label for="receipt-sort">정렬 기준</label><select id="receipt-sort" data-ui-select><option value="latest">최근 신청순</option><option value="oldest">오래된 신청순</option></select></div></div><p role="status">총 ${ordered.length}건</p>${ordered.length ? `<label class="choice"><input type="checkbox" id="select-all-receipts">현재 목록 전체 선택</label><div class="actions ui-form-actions"><button id="cancel-selected" class="danger">선택한 모의 신청 취소</button><button id="download-receipts">선택한 모의 내역 저장 (JSON)</button></div>${ordered.map(r => `<article class="receipt"><div class="receipt-select"><input id="select-${e(r.id)}" type="checkbox" data-receipt="${e(r.id)}"${selectedReceipts.has(r.id) ? ' checked' : ''}><label for="select-${e(r.id)}">${e(r.id)} 교육 참여 모의 신청 선택</label></div><h2><a href="#application/receipt/${e(r.id)}">교육 참여 모의 신청 · ${e(r.id)}</a></h2><p>${e(M.formatDate(r.createdAt))} · <strong>${e(r.status)}</strong></p><ol class="timeline" aria-label="처리 절차"><li aria-current="step">${e(r.status)}</li><li>실제 심사 없음</li><li>실제 처리 없음</li></ol><a href="#application/receipt/${e(r.id)}">${e(r.id)} 상세 내역 보기</a></article>`).join('')}` : '<div class="notice ui-alert"><h2>모의 신청 내역이 없습니다</h2><p>아직 신청하지 않았거나 현재 필터에 맞는 기록이 없습니다.</p></div>'}<div class="actions ui-form-actions"><a class="button primary" href="#application/eligibility">새 모의 신청</a><a class="button" href="#application">서비스 목록</a></div><div class="secondary-zone"><button id="clear-history" class="danger">이 예제의 저장 기록 모두 지우기</button><p>다른 사이트·다른 Altool 자산의 저장값은 지우지 않습니다.</p></div>`;
  ...
  function policy(stage, id) {
    ...
    main.innerHTML = pageHeading('정책 정보', '모든 가상 정책을 하나의 목록에서 대상·검색어로 좁혀볼 수 있습니다.', 948) + `<form id="policy-search" role="search"><div class="ui-filter-bar"><div class="field ui-field"><label for="policy-query">정책명</label><input id="policy-query" type="search" value="${e(policyQuery)}"></div><div class="field ui-field"><label for="policy-audience">정책 대상</label><select id="policy-audience" data-ui-select>${['전체', '청년', '중장년'].map(v => `<option>${v}</option>`).join('')}</select></div><div class="ui-filter-actions"><button class="primary">정책 검색</button><button type="button" id="policy-reset">조건 초기화</button></div></div></form><p role="status" aria-live="polite">총 ${data.length}건</p>${data.length ? `<div class="cards">${data.slice((policyPage - 1) * 3, policyPage * 3).map(item => `<article class="card ui-summary">${item.isNew ? '<span class="badge">신규 예제</span>' : ''}<span class="badge">대상 ${item.audience}</span><h2><a href="#policy/detail/${item.id}">${e(item.title)}</a></h2><p>${e(item.description)}</p><button data-bookmark="${item.id}" aria-label="${e(item.title)} 관심 정책 ${bookmarked.has(item.id) ? '해제' : '저장'}" aria-pressed="${bookmarked.has(item.id)}">${bookmarked.has(item.id) ? '관심 정책 해제' : '관심 정책 저장'}</button></article>`).join('')}</div><nav class="pagination ui-pagination" aria-label="정책 목록 페이지">${Array.from({ length: pages }, (_, i) => `<button data-policy-page="${i + 1}"${policyPage === i + 1 ? ' aria-current="page"' : ''}>${i + 1}</button>`).join('')}</nav>` : '<div class="notice ui-alert"><h2>조건에 맞는 정책이 없습니다</h2><p>다른 단어로 검색하거나 조건을 초기화해 주세요.</p></div>'}<div class="actions ui-form-actions"><a class="button" href="#policy/resources">정책 자료 탐색</a></div><p class="source-note">${source(951)}</p>`;
  ...
  function missing(message = '예제 항목을 찾을 수 없습니다. 목록에서 다시 선택해 주세요.') { main.innerHTML = `<h1>항목을 찾을 수 없습니다</h1><p class="notice ui-alert">${e(message)}</p><a class="button" href="#visit">홈으로</a>`; }
  function render() {
    ++requestVersion;
