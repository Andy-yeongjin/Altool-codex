...
def build():
 ...
 additions=json.loads((R/'icons/additions.json').read_text())
 out=R/'dist/assets';individual=out/'icons';individual.mkdir(parents=True,exist_ok=True)
 symbols=[];catalog=[]
 for item in [*source['icons'], *additions['icons']]:
 ...
 sprite='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">'+''.join(symbols)+'</svg>'
 ...
 shutil.copyfile(R/'icons/LICENSE-ALTOOL.txt',out/'LICENSE-ALTOOL.txt')
 manifest['additional_provider'] = additions['provider']
 manifest['additional_license_file'] = additions['license']
 (out/'icons.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2))
