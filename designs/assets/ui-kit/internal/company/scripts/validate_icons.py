...
sources=json.loads((R/'icons/source.json').read_text())['icons']+json.loads((R/'icons/additions.json').read_text())['icons']
ids=[i['id'] for i in M['icons']];assert len(ids)==len(set(ids))==M['count']==len(sources)
assert set(ids)=={i['id'] for i in sources}
...
assert (A/'LICENSE-ALTOOL.txt').read_text()==(R/'icons/LICENSE-ALTOOL.txt').read_text()
print(f'PASS: {len(ids)} IDs, SVG geometry, manifest paths, 1.5 stroke, licenses')
