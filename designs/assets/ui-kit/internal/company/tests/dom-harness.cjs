...
class Node extends EventTarget {
 ...
 cloneNode(deep=false){const node=new Node(this.tagName);node.attributes={...this.attributes};node._text=this._text;if(deep)node.append(...this.children.map(child=>child.cloneNode(true)));return node;}
...
document.createElementNS=(namespace,tag)=>{const node=new Node(tag);node.namespaceURI=namespace;return node;};
