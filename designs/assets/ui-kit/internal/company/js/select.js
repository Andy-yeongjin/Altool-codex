...
function enhance(select){
 ...
 const form=select.form,reset=()=>setTimeout(refresh,0);form?.addEventListener('reset',reset);
 ...
 const api={refresh,close,destroy(){close();observer.disconnect();document.removeEventListener('pointerdown',outside);window.removeEventListener('resize',position);window.removeEventListener('scroll',position,true);select.removeEventListener('change',refresh);select.removeEventListener('invalid',invalid);form?.removeEventListener('reset',reset);labelHandlers.forEach(([l,h])=>l.removeEventListener('click',h));if(select.parentElement===wrapper)wrapper.before(select);select.hidden=originalHidden;wrapper.remove();instances.delete(select);}};instances.set(select,api);refresh();return api;
}
