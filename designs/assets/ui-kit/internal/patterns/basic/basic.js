...
(function(){
  ...
  function filter(){
    ...
    on('filter-reset','click',()=>{$('filter-form').reset();$('filter-sort').value='newest';syncDistrict();applied=collect();page=1;message('filter-range-error','');render();text('filter-state','모든 필터와 정렬을 초기화했습니다.');});
    on('filter-inline-sort','click',()=>{applied.sort=applied.sort==='title-asc'?'title-desc':'title-asc';$('filter-sort').value=applied.sort;render();text('filter-state','제목 정렬을 적용했습니다.');});
