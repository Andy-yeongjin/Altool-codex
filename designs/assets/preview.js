...
function render() {
  ...
  for (const item of filtered) {
    const card = el('article', undefined, 'card' + (['messages','guideline'].includes(item.category) ? ' wide' : ''));
    ...
    } else { const art = el('div', undefined, 'art'); art.append(el('span', item.category === 'guideline' ? `p.${item.pages[0]}` : '</>', 'placeholder')); card.append(art); }
    ...
    if (item.category === 'guideline') info.append(el('p', `원문 p.${item.pages.join('–')} · ${item.implementation} / ${item.verification}`, 'meta'));
...
document.querySelectorAll('[data-source]').forEach(button => button.addEventListener('click',()=>{
  source=button.dataset.source; assets=source==='adopted'?adopted:references;
...
Promise.all(['catalog.json','messages/ko.json','ui-kit/internal/coverage.json','registry.json','pack.lock.json'].map(async path=>{
  ...
  const [catalog, messages, coverage, registry, lock]=results.map(result=>result.data);
  for (const [index,path] of [[3,'registry.json'],[1,'messages/ko.json']]) {
  ...
  adopted=registry.items.filter(item=>!item.restrictedTo||item.restrictedTo.includes('company')).map(item=>{
    ...
    return {id:item.id,semantic:true,category:categories[item.kind],name:item.meaning,path:value.path.replace(/^designs\/assets\//,''),
      preview:(value.preview || (/\.html$/.test(value.path)?value.path:undefined))?.replace(/^designs\/assets\//,''),message:item.kind==='message'?messages.messages[value.key]:undefined,
      source:`${registry.pack}@${registry.release}`,defaultVariant:item.defaultVariant,variantNames:Object.keys(item.variants)};
  });
  references=[...catalog.assets,...Object.entries(messages.messages).map(([id,message])=>({id,category:'messages',name:message.title,path:'messages/ko.json',message}))];
  assets=adopted;
  ...
  const mapped=coverage.items.filter(item=>item.assets.length).length;
  const reviewed=coverage.pages.filter(page=>page.visualReview==='source-visually-reviewed').length;
  document.querySelector('#coverage').textContent=`페이지 색인 ${coverage.pages.length}/${coverage.source.pageCount} · 원문 시각 대조 ${reviewed}/${coverage.source.pageCount} · 목차 ${coverage.source.tocCount}항목 · 자산 연결 ${mapped}개 · 회사 적용·전체 규칙 준수 검증은 별도`;
  render();
}).catch(()=>{
  const error=document.querySelector('#error');error.hidden=false;error.textContent='자산 목록 또는 회사 팩을 확인하지 못했어요. 로컬 HTTP 서버와 registry·lock·catalog·메시지·coverage 파일을 확인하고 assets.py validate를 실행해 주세요.';
