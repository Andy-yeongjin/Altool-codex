...
### 제16조: 디자인 기준 우선순위와 품질 원칙 (Design Source Priority & Quality)
UI 구현의 시각 기준은 아래 순서로 결정합니다.

| 우선순위 | 원천 | 역할 |
|------|------|------|
| 1 | 사용자 디자인 입력 (`designs/*.pen`, `designs/stitch/`, 스크린샷, 디자인 문서) | 화면 구조, 정보 밀도, 시각 위계, 컴포넌트 외형의 최우선 원천 |
| 2 | 프로젝트 디자인 시스템 (`designs/design.md`) | 반복 가능한 디자인 값, 컴포넌트 규칙, 레이아웃 원칙, 미디어 규칙의 원천 |
| 3 | Research 기반 디자인 시스템 생성 | 사용자 디자인 입력과 디자인 시스템이 없거나 빈칸이 있을 때 `design.md`를 생성·보강하는 근거 |
| 4 | AI 자체 판단 | 위 원천으로도 결정되지 않는 세부 보완에만 사용 |

프로젝트에 `design.md`가 존재하는 경우, 모든 UI 요소는 `design.md`에 정의된 디자인 명세를 따릅니다. `design.md`는 색상, 타이포그래피, 간격, 둥글기, 그림자 같은 시각 값뿐 아니라 컴포넌트 계약, 페이지 구조, 미디어 사용 규칙, Do/Don't를 함께 담는 단일 디자인 원천입니다. 구현 시 CSS 변수나 CSS Module 상수는 코드 내부에서 `design.md`를 근거로 파생할 수 있지만, 별도 디자인 원천으로 취급하지 않습니다.

프로젝트에 `design.md`가 없거나 비어 있거나 첫 non-empty line에 `TBD`가 있으면 UI를 구현하기 전 Research 단계에서 참조 사이트와 사용자 디자인 입력을 근거로 `design.md`를 먼저 생성합니다. Research는 색상, 타이포, 폰트 성격과 안전한 구현 폰트 스택, 간격, 그림자, 둥글기, 컴포넌트 외형 값, 페이지 구조, 미디어 규칙을 조사할 수 있지만, 그 값은 직접 구현 지시가 아니라 프로젝트 디자인 시스템을 만들기 위한 원천입니다. 참조 사이트의 폰트 파일은 복제하지 않고, 시스템 폰트·오픈 라이선스 폰트·사용자 제공 폰트·프로젝트 포함 폰트만 구현에 사용합니다. 완성된 프로젝트별 `design.md`는 최상단 `TBD` 마커를 제거해야 합니다. 단, 문장·카피·로고·고유 이미지는 그대로 복제하지 않습니다.

모든 UI는 원천과 무관하게 다음 품질 원칙을 지켜야 합니다.

- 사용자가 내용을 읽고 이해할 수 있도록 충분한 대비, 크기, 간격, 정보 위계를 확보합니다.
- 클릭·터치·키보드 조작 대상은 실수 없이 사용할 수 있을 만큼 명확한 크기와 상태 표현을 가져야 합니다.
- 모든 핵심 기능은 키보드와 보조기술로 접근 가능해야 하며, 의미 있는 HTML 구조와 명확한 포커스 상태를 제공합니다.
- 화면 크기가 달라져도 텍스트와 UI가 겹치거나 잘리거나 조작 불가능한 상태가 되지 않아야 합니다.
- 로딩, 빈 상태, 오류, 성공, 비활성 상태는 사용자가 다음 행동을 판단할 수 있게 명확히 표현합니다.
- 장식보다 실제 사용 흐름, 정보 탐색, 반복 작업의 효율을 우선합니다.
- 사용자가 제공한 디자인 원천과 충돌하지 않는 한, 제품 도메인과 사용 맥락에 맞는 시각 표현을 선택합니다.

### 제17조: 사용자 디자인 원본 준수 (Design Source Fidelity)
프로젝트에 `.pen`, Stitch, 스크린샷, 디자인 문서가 제공된 경우, 해당 파일은 UI의 **레이아웃·구조·컴포넌트 배치·시각 위계의 원본**입니다. AI와 개발자는 사용자 디자인 원천에 정의된 화면 구성, 컴포넌트 배치 순서, 정보 계층 구조를 충실히 재현해야 하며, 임의로 레이아웃을 변경하거나 재해석하지 않습니다. `design.md`가 있고 첫 non-empty line에 `TBD`가 없으면 시각적 수치(높이·색상·폰트 크기·간격·둥글기·그림자 등)는 제16조의 디자인 시스템으로 매핑하여 구현합니다. 디자인 시스템이 없거나 `TBD` 템플릿이면 Research 또는 design_source 단계에서 사용자 디자인 원천을 `design.md`로 정규화한 뒤 구현합니다.

**역할 분리 요약:**
| 원본 | 역할 | 예시 |
|------|------|------|
| 사용자 디자인 입력 | **구조·시각 원본** — 무엇이 어디에 어떻게 배치되는가 | 화면 구성, 컴포넌트 순서, 정보 계층, 인터랙션 흐름, 시각 위계 |
