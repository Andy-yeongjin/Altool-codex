...
이 문서는 프로젝트 자산 사용 안내입니다. 공통 아이콘을 선택할 때 `dist/assets/icons.json`을 먼저 읽습니다. 외부 패키지 설치나 네트워크 요청은 필요하지 않습니다.

## 선택 순서
1. `id`, `label_ko`, `keywords`, `category`로 의미가 맞는 아이콘을 찾습니다.
...
- 배포 시 라이선스도 포함합니다: `dist/assets/LICENSE-LUCIDE.txt`.
...
## 버전 및 재생성
Lucide 1.8.0에서 선별한 120개입니다. 원본 노드와 선정 정보는 source.json, selection.tsv에 보관했습니다. 원본의 선 두께를 D 규격 1.5로 조정했습니다. 라이선스 전문은 LICENSE-LUCIDE.txt에 포함됩니다.

`python3 scripts/build_design.py`는 CSS와 아이콘을 함께 재생성합니다. 아이콘만 재생성할 때는 `python3 scripts/build_icons.py`를 실행합니다. 원본 데이터가 포함되어 있어 Lucide 패키지를 다시 설치할 필요가 없습니다.
